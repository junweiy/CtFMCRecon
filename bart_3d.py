...
if __name__ == '__main__':
    ...
    parser.add_argument('--reg', type=str, default='TV', help='regularizer, TV or wavelet')
    ...
    for curr_image_file in file_list:
        ...
        t2_img = np.fft.ifft2(np.fft.ifftshift(t2_k))[0]
        t2f_img = np.fft.ifft2(np.fft.ifftshift(t2f_k))[0]

        t2_k = np.fft.fftshift(np.fft.fftn(t2_img, axes=(0, 1, 2)), axes=(0, 1, 2))
        t2f_k = np.fft.fftshift(np.fft.fftn(t2f_img, axes=(0, 1, 2)), axes=(0, 1, 2))
        ...
        if opts.reg == 'TV':
            recon = bart(1, f'pics -d5 -S -i 100 -R T:7:0:{opts.alpha}', inp, sens)
        elif opts.reg == 'wav':
            recon = bart(1, f'pics -d5 -S -i 100 -R W:7:0:{opts.alpha}', inp, sens)
        else:
            raise NotImplementedError

        recon /= abs(recon).max()
        recon = abs(np.fft.ifftshift(recon, axes=(0, 1, 2)))
