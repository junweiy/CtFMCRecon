...
def cartesian_mask(shape, rate, sample_n=10, centred=True):
    assert rate > 1
    if len(shape) == 4:
...
def get_vd_mask(shape, rate, seed=0, square_area=0):
    if len(shape) == 2:
        W, H = shape
    else:
        raise ValueError("Invalid dimension, {} given.".format(len(shape)))
    mask = poisson((W, H), rate, dtype=np.int, crop_corner=False)
    mask[W // 2 - square_area // 2: W // 2 + square_area // 2, H // 2 - square_area // 2: H // 2 + square_area // 2] = 1
    return mask
